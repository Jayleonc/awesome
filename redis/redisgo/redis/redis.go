...
const (
	NodePrefix    = "registry_node"    // key的前缀
	GatewayPrefix = "registry_gateway" // key的前缀
)

//// New 根据配置参数创建redis工具实例
//func New(Prefix string) *Cacher {
//	r := &Cacher{}
//	r.Pool = orm.RedisPool
//	r.Prefix = Prefix
//	return r
//}
...
func (c *Cacher) HSetNx(key, field string, val interface{}) (interface{}, error) {
	value, err := c.Encode(val)
	if err != nil {
		return nil, err
	}
...
// HValues 获取哈希表中所有值
func (c *Cacher) HValues(key string, val interface{}) ([]interface{}, error) {
	reply, err := redis.Values(c.Do("HVALS", c.getKey(key)))
	if err != nil {
		return nil, err
	}
	return reply, nil
...
// HExists 判断指定field是否存在, reply: 1-存在||0-不存在
func (c *Cacher) HExists(key, field string) (reply int64, err error) {
	reply, err = Int64(c.Do("HEXISTS", c.getKey(key), field))
	return
}

// HDel 删除指定field, reply: 1-成功删除的个数||0-删除失败
func (c *Cacher) HDel(key, field string) (reply int64, err error) {
	reply, err = Int64(c.Do("HDEL", c.getKey(key), field))
	return
}
...
func (c *Cacher) ZAdd(key string, score int64, member interface{}) (reply interface{}, err error) {
	// 序列化member数据
	res, err := c.Encode(member)
	if err != nil {
		return nil, err
	}
	return c.Do("ZADD", c.getKey(key), score, res)
}
